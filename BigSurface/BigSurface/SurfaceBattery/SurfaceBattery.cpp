...
void SurfaceBattery::updateInfoExtended(OSArray *bix) {
    ...
    // Fallback if hardware reports empty manufacturer
    if (bi.manufacturer[0] == '\0') {
        strncpy(bi.manufacturer, "Microsoft", BatteryInfo::MaxStringLen);
    }
    if (bi.batteryType[0] == '\0') {
        strncpy(bi.batteryType, "Li-ion", BatteryInfo::MaxStringLen);
    }
