...
SMC_RESULT IB0R::readAccess() {
    SInt16 total_current = 0;
    IOSimpleLockLock(BatteryManager::getShared()->stateLock);
    for (int i = 0; i < BatteryManager::getShared()->batteriesCount; i++) {
        if (BatteryManager::getShared()->state.btInfo[i].connected) {
            total_current += BatteryManager::getShared()->state.btInfo[i].state.signedPresentRate;
        }
    }
    IOSimpleLockUnlock(BatteryManager::getShared()->stateLock);
    
    UInt16 *ptr = reinterpret_cast<UInt16 *>(data);
    *ptr = VirtualSMCAPI::encodeSp(SmcKeyTypeSp78, (double)total_current / 1000.0);
    return SmcSuccess;
}

SMC_RESULT PB0R::readAccess() {
    double total_power = 0;
    IOSimpleLockLock(BatteryManager::getShared()->stateLock);
    for (int i = 0; i < BatteryManager::getShared()->batteriesCount; i++) {
        if (BatteryManager::getShared()->state.btInfo[i].connected) {
            double current = (double)BatteryManager::getShared()->state.btInfo[i].state.presentRate / 1000.0; // A
            double voltage = (double)BatteryManager::getShared()->state.btInfo[i].state.presentVoltage / 1000.0; // V
    ...
    UInt16 *ptr = reinterpret_cast<UInt16 *>(data);
    *ptr = VirtualSMCAPI::encodeSp(SmcKeyTypeSp78, total_power);
    return SmcSuccess;
}
